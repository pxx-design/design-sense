...
**一屏一模块（编排规则 · 2026-07 官网实测）**：叙事区段以**视口为构图单位**——一个区段 = 一个论点 + 它的证据 + 一个出口，高度贴着 ~1 屏走（sandbox 实测九段中七段落在 0.78-1.25 屏；hero 0.93-1.0 屏）。滚动停点和论点边界对齐，这是「疏密有意」（及格 Q6）在落地页上的实现方式。
**例外**：查阅型内容（价格矩阵、FAQ、对比表）不按屏分（官网 pricing 矩阵一段 12.6 屏），靠分组 + 局部密度维持扫读舒适；内容超长时编辑截断——区段放代表性切片 +「View all」出口，不放全量清单。
