...
> 风格之前，先及格。任何风格都站在同一套排版标准上——复古编辑不是这套标准的例外，是它的一种「怎么达成」。
> 生成后**能渲染就渲染出来看一眼**，对着下面七问自检；**认出失格，就回去补，别用装饰盖过去。**

### 及格七问 · 每条都教你「失格长什么样」

| 判准 · 扫一眼问自己 | 失格的样子（要认出的 tell） | 复古编辑靠什么达成 |
|---|---|---|
| **1 层级**：这一屏有没有一个明确的第一落点？（逐屏问，不是全页平均） | 同屏两三个东西一样重，没有主角 | serif display 峰（全页唯一，享唯一的居中特权）+ 每屏至少一处墨色锚——不靠字号阶梯、不靠彩色 |
| **2 亲密**：间距在说「一组 / 分块」吗？ | 到处等距，或忽宽忽窄没逻辑，看不出谁跟谁一组 | 间距按结构角色分三档 + 1px 发丝边框分组；染料浅卡是图版区的分组容器 |
| **3 对齐**：都贴着同一套隐形线吗？ | 边缘游离，东西没贴任何线 | 左对齐默认（能扫读的永不居中）；长文 640px 阅读列一根轴贯穿到底 |
| **4 对比**：要不同，够不够决绝？ | 近似双胞胎（15 挨 16、灰度只差一点），看着像噪声 | serif↔sans 是两种声音，切换要决绝；明暗混排 1 深 2 浅；档少、每档拉开 |
| **5 一致**：同角色同待遇吗？ | 同类卡片这里 padding 一个样、那里一个样 | 同结构角色 → 同一个角色 token；「结构标题恒定 24px w600 不逐级放大」正是此条的极致 |
| **6 节奏**：疏密是有意的吗？ | 一种密度到底，像填表格（文档载体最容易犯） | 奢侈留白（10rem 级）造谷 + 墨色锚（深卡/黑按钮/重墨题）造重音；克制 ≠ 均匀 |
| **7 可读**：行长 / 行距 / 对比够吗？ | 正文行拉太宽、行距太挤、灰得看不清 | 长文 serif 640px 单列（~75 拉丁字符/~35 汉字）；中文正文苹方行距 ~1.7；**行长与行距联动**，不是死数 |

### 咬合纪律 · 全篇的骨（动杆必补及格）

复古编辑是靠「**拿掉**」定义自己的：拿掉阴影（0 shadow）、拿掉字号阶梯（标题不逐级放大）、拿掉功能色系统（颜色只住荧光笔）、拿掉动效表演（纸不浮起）。

> **每拿掉一个工具，它原本能承担的那条及格，就必须由留下的工具补上——否则就是「套了皮、丢了骨」。**

- 拿掉了「阴影做层级」 → 层级**全压到明暗混排（1 深 2 浅）+ 1px 边框 + 墨色锚**。所以每屏必须有一处重墨——不是偏好，是在补及格。
- 拿掉了「字号阶梯做标题层级」 → 层级**全压到 serif↔sans 切换 + 字重 + 位置**。所以两种声音的切换必须决绝，含糊就一片平。
- 拿掉了「彩色做状态 / 分组」 → 分组**全压到间距 + 发丝线 + tag 三态**；强调压到批注双笔（下划线 + 软染料 pill）。
- 拿掉了「动效抓注意力」 → 节奏**全压到留白与明暗的编排**。

**反向自检（最常踩的坑）**：你把 Anthropic 的皮套上了——暖纸、衬线、染料色都有——但页面**一片平、没锚、像套模板的文档**？那就是**拿掉了工具却没补及格**。回去用墨色锚和奢侈留白把层级挣回来，**别加装饰回来**。

### 角色词汇 · 咬合的执行面（禁裸值）

字号和间距**不许裸写 px**——全部挂角色（值是克制默认，见 tokens.css；载体差异见 layouts.md）：

**声部六种**（对应 tokens.css 现成类；这门语言的层级靠声部切换，不靠字号爬梯）：

| 角色 | 类 / token | 纪律 |
|---|---|---|
| display 峰 | `.text-display-serif` | **全页唯一**；唯一的居中特权 |
| 结构标题 | `.text-h-sans`（24px w600） | 恒定，不逐级放大 |
| 条目标题 | ≈正文字号 0.9-1.2× | 靠 serif↔sans / 字重 / 位置区分，绝不放大喊叫 |
| 长文正文 | serif 17-20px | 640px 阅读列 |
| UI / 说明 | `.text-ui` sans | 短文本、表格、chrome |
| meta / 小标 | `.text-detail` / mono caps | 全大写微正字距 |

**间距三档**：`--gap-element`（组内）< `--gap-block`（块间）< `--gap-section`（区段间，10rem 级）——三档差距要看得出（Q2 的结构化）。

**动笔前先写角色清单**：这页的峰是谁、几个区段、哪些块、墨色锚落在哪几屏——先声明，再写 CSS。角色分错时所有值都「合法」、任何校验器都结构性看不见，清单是唯一把这步判断亮出来的地方。真需要第七种声部？先写一行理由再加（护栏，不是禁令）。

*具体数字（字阶值、间距档位、量化阈值）不在这一层——它们退到「参数参考」（tokens.css + 字距规则）当后台默认。这一层只教「看懂什么叫踩住了标准」。*
...
- ❌ **亮饱和科技色 / 霓虹** —— 颜料盒是天然染料，饱和天花板 ~55%
  *为什么：物理染料天生不会 neon——所有彩色必须在 CMYK 里有对应颜色（见「CMYK 闸」节：新增色先写油墨配方再取 hex）*
...
## CMYK 闸 · 先油墨，后 hex（颜色的物理学）

**这门语言的颜色先存在于油墨里，然后才上屏。** 屏幕只是在模拟一张被印过的纸——所以**所有彩色（染料、clay、数据色，以及任何新增色）都必须在 CMYK 里有对应颜色**。这是"天然染料 / 饱和封顶 ~55% / 无霓虹"这些规则共同的物理根。

**入闸仪式（钉死）**：新增任何彩色，落笔前写一行 CMYK 近似配方（naive 换算即可，不用色彩管理软件）——

```
clay    #d97757 ≈ C0  M45 Y60 K15   （赭石橙——烧过的陶土）
heather #cbcadb ≈ C7  M8  Y0  K14   （淡堇——石南染）
```

写不出"像一支油墨"的配方——那种**要靠屏幕发光才成立**的颜色——不进页面。选色顺序永远是：先想"这是什么染料 / 油墨"，再转 hex；不是屏幕上挑个艳的再"压暗"。

**快问**：这颜色能用油墨印在这张暖纸（#faf9f5）上而不刺眼吗？
**数值护栏（辅助判断，非硬闸）**：HSB 里 S > 60% 且 B > 90% 的"会发光"色（#39ff14 荧光绿、#00ffff 电光青一类）基本超出印刷色域——出现即嫌疑。
**分层（A6 口径）**：钉死＝先油墨后 hex + 新增色必附配方；克制默认＝配方精度（近似即可）；自由＝具体选哪支染料。
...
| 插图 | 三个 register 按内容选：论点有结构（对比/象限/前后）→ **论点图解**（墨线线稿，见 layouts.md#论点图解）；纯概念气质 → 手绘线描（老物件隐喻 + 留抖）；hero 级才升格点描/博物 plate 且全站罕见 |
...
- [ ] **动笔前写了角色清单**（峰是谁 / 几个区段 / 哪些块 / 墨色锚落在哪几屏）；字号 / 间距无裸值——全部挂声部角色类与 `--gap-*` 三档
- [ ] **display 峰全页唯一**，且它那一屏没有第二个抢落点的东西
- [ ] **全页彩色逐一过 CMYK 闸**：每支彩色写得出油墨配方，无"要靠屏幕发光才成立"的颜色
...
能渲染就渲染出来看一眼，对照顶部「先及格」七问逐条认失格。认出失格 → **回角色 token 重推**，用墨色锚和留白把层级挣回来；**不加装饰盖过去、不挪 1px 凑数**。
...
| 想看「好的分配」长什么样（峰与锚 / 目录三式 / register 对照 / 读） | [`specimen.md`](specimen.md)（官网六现场带批注 + 实测真值）|
...
**Version**: v1.2 · 2026-07-10（内部验证中，未发布）
**Skill 整理**: shona · TRUE NAME STUDIO

v1.2（2026-07-10，内部）：顶部新增「先及格，再风格」层（及格七问 + 咬合纪律 + 角色词汇/禁裸值——七问判准与失格 tell 两列为全 DNA 共享正典，与 dna-vercel 逐字同源，仅"靠什么达成"列不同）；Pre-flight 加角色清单 / 唯一峰两项 + 独立「渲染目检·七问」步骤；新增 **CMYK 闸**（所有彩色须有油墨对应，新增色先写 CMYK 配方——shona 品味裁定：用于软件的颜色用的是印刷色）；插图新增**论点图解** register（线稿谱系，源自 TRUE NAME STUDIO fable field-guide 卡片实作，见 layouts.md）；新增 specimen.md 官网六现场带批注范例（峰与锚 / 产品层对照 / 目录三式 / 读，shona 选页，2026-07-10 实测）；tokens.css 新增 --gap-* 间距三档。
